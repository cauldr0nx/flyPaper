...
| `bench-sprawl` | 1998 | 326 [16-1585] | 776 [15-1998] | 652 [13-1986] | **512** | 690 [17-1591] |
...
| `bench-sprawl` | 512 | 14/32, p=0.455 | 12/32, p=0.394 | 13/32, p=0.424 |
...
Discriminating surfaces (those where the seed changes the answer): `bench-collide`, `bench-mixed`, `bench-sprawl`, `bench-token`.

| Null | draws matching the connectome on all of them | p |
|---|---:|---:|
| `random on v3-response` | 3/32 | 0.121 |
| `random` | 1/32 | 0.061 |
| `random-matched` | 3/32 | 0.121 |
| `connectome-permuted` | 3/32 | 0.121 |
...
| `bench-sprawl` | 7 | 4.00 | 5.00 | 5.00 | 5.00 |
...
**The connectome beats a plain random projection, and does not beat its own degree-preserving null.** Held across every discriminating surface at once, 1 of 32 uniform random draws match it (p=0.061); 3 of 32 draws that keep the measured *fan-in* while randomising the *targets* match it (p=0.121), as do 3 of 32 that keep the wiring and shuffle which response feature feeds which glomerulus (p=0.121). Two independent controls agree: nothing detectable here comes from which glomerulus reaches which Kenyon cell.
...
**What shipped: nothing.** Not the connectome - it needs a 508 MB download, a 55-channel encoder that is worse on its own, and it does not beat a random projection carrying its fan-in distribution. And not the fan-in distribution either. That looked like the one thing here a random projection would not have suggested, and it was written up as such; then it was tested on a second, independently built heterogeneous surface and did not reproduce, and the result it was based on turned out not to survive a paired test on its own surface. `--projection degree-sampled` exists and is not recommended. See [claw-degrees.md](claw-degrees.md), which retracts it.
...
*Generated by flypaper 0.1.0 at commit `fa16d8dd0009f28320590ec383bbf22ab567cdc3` **(dirty working tree - not reproducible)** | seed `0` on 2026-09-12T03:04:12+00:00.*
